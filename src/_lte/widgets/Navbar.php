...
class Navbar extends \kordar\yak\widgets\YakWidget
{
    ...
    {
        return $this->beforeRender() . strtr('<nav class="navbar navbar-static-top">{toggle}<div class="navbar-custom-menu">
    ...
    {
        $shortTitle = '<span class="logo-mini">' . mb_substr($this->config['title'], 0, 3) . '</span>';
        return Html::a($shortTitle . '<span class="logo-lg"><b>' . $this->config['title'] . '</b></span>',
    ...
    protected function personal()
    {
        ...
        $a = Html::a(Html::img($user->getAvatar($defaultAvatar), ['class' => 'user-image']) . '<span class="hidden-xs">' . $user->getName() . '</span>', '#', ['class' => 'dropdown-toggle', 'data-toggle' => 'dropdown']);
